...
-- Tạo AdministrativeStaff tạm (hash trong DB). Chạy whole block (không có GO giữa các dòng)
DECLARE
  @plain NVARCHAR(4000) = N'StaffPassword123!',   -- mật khẩu mạnh hơn cho nhân viên hành chính
  @salt VARBINARY(32) = CRYPT_GEN_RANDOM(32),
  @hash VARBINARY(MAX),
  @i INT = 0,
  @iterations INT = 1000,
  @staffUserID UNIQUEIDENTIFIER,
...
SELECT @staffRoleID = RoleID FROM dbo.Roles WHERE RoleName = N'AdministrativeStaff';

INSERT INTO dbo.UserRoles (UserID, RoleID, AssignedAt, AssignedBy)
VALUES (@staffUserID, @staffRoleID, SYSUTCDATETIME(), NULL);
...
-- Tạo ExecutiveStaff tạm (hash trong DB). Chạy whole block (không có GO giữa các dòng)
DECLARE
  @plain NVARCHAR(4000) = N'ExecutivePassword123!',   -- mật khẩu mạnh cho nhân viên điều hành
  @salt VARBINARY(32) = CRYPT_GEN_RANDOM(32),
  @hash VARBINARY(MAX),
  @i INT = 0,
  @iterations INT = 1000,
  @execUserID UNIQUEIDENTIFIER,
  @execRoleID UNIQUEIDENTIFIER;
...
SET @execUserID = NEWID();

INSERT INTO dbo.Users (UserID, Username, Email, PasswordHash, PasswordSalt, PasswordAlgo, CreatedAt, FullName)
VALUES (
  @execUserID,
  N'execstaff',
  N'execstaff@example.com',
  @hash,
  @salt,
  N'SHA2_512+iter1000',
  SYSUTCDATETIME(),
...
SELECT @execRoleID = RoleID FROM dbo.Roles WHERE RoleName = N'ExecutiveStaff';

INSERT INTO dbo.UserRoles (UserID, RoleID, AssignedAt, AssignedBy)
VALUES (@execUserID, @execRoleID, SYSUTCDATETIME(), NULL);
